# -*- ruby -*-
# autogenerated via `brew gem formula theme-check`
...
class ThemeCheck < Formula
  ...
  def install
    ...
    raise "gem install 'theme-check' failed with status #{$?.exitstatus}" unless $?.success?
    ...
    completion_for_bash = Dir[
                            "#{brew_gem_prefix}/completion{s,}/theme-check.{bash,sh}",
                            "#{brew_gem_prefix}/**/theme-check{_,-}completion{s,}.{bash,sh}"
                          ].first
    bash_completion.install completion_for_bash if completion_for_bash

    completion_for_zsh = Dir[
                           "#{brew_gem_prefix}/completions/theme-check.zsh",
                           "#{brew_gem_prefix}/**/theme-check{_,-}completion{s,}.zsh"
                         ].first
